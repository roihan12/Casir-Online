...
const InvoiceDetail = () => {
  ...
  const handlePrintInvoice = async () => {
    try {
      setIsPrinting(true);
      // We can reuse the generatePdf logic or call the API directly
      // Since generatePdf from hook is bound to refetch, let's use a direct API call mechanism 
      // or we can use the existing hook if we modify it to return data on demand, 
      // but simpler to just fetch blob here for printing specific action to avoid state conflict
      ...
      iframe.onload = () => {
        // Wait a bit for PDF to render in iframe
        setTimeout(() => {
          iframe.contentWindow.print();
          
          // Cleanup after print dialog closes (approximate) or user interaction
          // setTimeout(() => {
